...
async function sendEmail(
  ...
  const from =
    settings?.email_from || process.env.RESEND_FROM || process.env.CONTACT_FROM_EMAIL || "";
  ...
  if (!to) {
    await log(
      "email",
      "error",
      "Recipient address not configured (set Admin → Notifications email_to or CONTACT_INBOX_EMAIL).",
      {},
    );
    return;
  }
  if (!from) {
    await log(
      "email",
      "error",
      "From address not configured (set Admin → Notifications email_from or RESEND_FROM/CONTACT_FROM_EMAIL).",
      {},
    );
    return;
  }
